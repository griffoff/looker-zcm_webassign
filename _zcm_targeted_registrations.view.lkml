...
view: _zcm_targeted_registrations {
    view_label: "          Targeted Redesign Registrations"
    derived_table: {
      sql:
      SELECT
                DISTINCT r.dim_school_id as dim_school_id    -- Pulling from registrations not school to insure that only institutions with registrations are pulled in
      ...
      FROM WEBASSIGN.FT_OLAP_REGISTRATION_REPORTS.FACT_REGISTRATION  AS r
          ...
          INNER JOIN ${dim_time_zcm.SQL_TABLE_NAME} AS time ON r.DIM_TIME_ID = time.DIM_TIME_ID
            ...
            AND (time.ay_value >= -{% parameter _zcm_cg_registrations.date_range_ay %})
                ;;
    }
